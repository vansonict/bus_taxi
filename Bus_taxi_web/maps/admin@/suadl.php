<?php 
	...
	if(isset($_SESSION['type_user']) && $_SESSION['type_user'] != 2){
		if(isset($_GET['parent_id'],$_GET['name'],$_GET['mobile'])){
				...
				$re = $c_student -> capnhat($_GET['id'], $fullName, $_GET['parent_id'], $_GET['pass'], $_GET['mobile'], $_GET['contact']);
			if(!$re){
		...
		$rs = $c_student -> chi_tiet_ten($_REQUEST['name']);
		...
		if(form.name.value==""){
			...
			return false;
                ...
                if(form.name.value.length != 2){
                    alert("Tên đại lý xin vui lòng nhập 2 kí tự!");
                    document.form.name.focus();
                    return false;
                }
    	...
    	<table class="table_full" border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr>
                ...
                <td bgcolor="#F2F2F2" valign="top">
                	...
                	<!--Nội dung chính -->
                    <div class="info_tab"><span class="cell_left">Sửa thông tin đại lý</span>
								 ...
								 <form action="" name="form" method="GET" class="niceform" onSubmit="return checkinput()">
										<fieldset>
                                                                                        ...
                                                                                        <?php 
                                                                                            $select = '';
                                                                                            $input_name = '';
                                                                                            $kt = $c_student->chi_tiet_parent($rs['name']);
                                                                                            if($kt){
                                                                                                  $select = 'disabled="disabled"';
                                                                                                  $input_name = 'readonly="readonly"';
                                                                                            }
                                                                                        ?>
                                                                                        <dl>
                                                                                                <dt><label>Đại lý cha :</label></dt>
                                                                                                <dd>
                                                                                                    <select <?php echo $select;?>size="1" name="parent_id" id="">
                                                                                                        <?php 
                                                                                                        if($rs['parent_id'] != '0')
                                                                                                            echo '<option value="00">00</option>';
                                                                                                        foreach ($pr as $pr){
                                                                                                            if($pr['name'] === $_REQUEST['name']){
                                                                                                                echo "<option selected=\"selected\" value=". $pr['parent_id'] .">". $pr['parent_id'] ."</option>";
                                                                                                            }elseif($pr['name']  <= 9999)  //Không phải là đại lý cấp 3
                                                                                                                echo "<option value=". $pr['name'] .">". $pr['name'] ."</option>";
                                                                                                        }
                                                                                                        $subname = substr($rs['name'], -2);
                                                                                                        ?>
                                                                                                    </select>
                                                                                                </dd>
                                                                                        </dl>
                                                                                        <dl>
                                                                                                ...
                                                                                                <dd><input <?php echo $input_name;?>type="text" name="name" onBlur="if(this.value=='') this.value='<?php echo $subname;?>'" value="<?php echo $subname; ?>" id="" size="54" /></dd>
                                                                                        </dl>
                                                                                        <dl>
												<dt><label for="pass">Password :</label></dt>
                                                                                                <dd><input type="password" name="pass" onBlur="if(this.value=='') this.value='<?php echo $rs['password'];?>'" value="<?php echo $rs['password']; ?>" id="" size="54" /></dd>
											</dl>
											<dl>
												<dt><label for="mobile">Số điện thoại :</label></dt>
												<dd><input type="text" name="mobile" id="" onBlur="if(this.value=='') this.value='<?php echo $rs['mobile'];?>'" value="<?php echo $rs['mobile'];?>" size="54" /></dd>
											</dl>
											<dl>
												<dt><label for="contact">Liên hệ :</label></dt>
												<dd><input type="text" name="contact" id="" onBlur="if(this.value=='') this.value='<?php echo $rs['contact'];?>'" value="<?php echo $rs['contact'];?>" size="54" /></dd>
											</dl>
											<dl class="submit">
